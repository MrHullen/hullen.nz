<script>
  import { page } from '$app/stores';
  import { activeSection, isActive } from '$lib/navStore';
  import { cleanRead } from '$lib/accessibilityStore';
</script>

  <!-- Top Navigation -->
  <nav class="flex justify-between items-center px-6 h-16 w-full fixed top-0 z-[10000] bg-[#131313] shadow-[0_4px_0_0_#00fbfb]">
    <div class="text-2xl font-black italic tracking-tighter text-[#00fbfb] drop-shadow-[0_0_8px_rgba(0,251,251,0.6)] font-headline">HULLEN.NZ</div>
    <div class="hidden md:flex gap-8 items-center">
      <a class="font-['Space_Grotesk'] tracking-tighter uppercase text-sm active:translate-y-0.5 active:translate-x-0.5 {isActive('system', $page.url.pathname, $activeSection) ? 'text-[#fe00fe] border-b-4 border-[#fe00fe] pb-1' : 'text-[#00fbfb] opacity-80 hover:opacity-100 hover:bg-[#00fbfb] hover:text-[#131313] transition-colors duration-75'}" href="/">ROOT</a>
      <a class="font-['Space_Grotesk'] tracking-tighter uppercase text-sm active:translate-y-0.5 active:translate-x-0.5 {isActive('minis', $page.url.pathname, $activeSection) ? 'text-[#fe00fe] border-b-4 border-[#fe00fe] pb-1' : 'text-[#00fbfb] opacity-80 hover:opacity-100 hover:bg-[#00fbfb] hover:text-[#131313] transition-colors duration-75'}" href="/#minis">CORE</a>
      <a class="font-['Space_Grotesk'] tracking-tighter uppercase text-sm active:translate-y-0.5 active:translate-x-0.5 {isActive('code', $page.url.pathname, $activeSection) ? 'text-[#fe00fe] border-b-4 border-[#fe00fe] pb-1' : 'text-[#00fbfb] opacity-80 hover:opacity-100 hover:bg-[#00fbfb] hover:text-[#131313] transition-colors duration-75'}" href="/#code">PROJECTS</a>
      <a class="font-['Space_Grotesk'] tracking-tighter uppercase text-sm active:translate-y-0.5 active:translate-x-0.5 {isActive('logs', $page.url.pathname, $activeSection) ? 'text-[#fe00fe] border-b-4 border-[#fe00fe] pb-1' : 'text-[#00fbfb] opacity-80 hover:opacity-100 hover:bg-[#00fbfb] hover:text-[#131313] transition-colors duration-75'}" href="/#logs">ARCHIVE</a>
    </div>
    <div class="flex items-center gap-4 text-[#00fbfb]">
      <button 
        on:click={() => $cleanRead = !$cleanRead} 
        aria-label="Toggle Clean Read Mode" 
        title="Toggle Clean Read Protocol"
        class="text-[#00fbfb] hover:text-[#fe00fe] transition-colors flex items-center justify-center"
      >
        <span class="material-symbols-outlined text-xl">
          {$cleanRead ? 'visibility' : 'visibility_off'}
        </span>
      </button>
      <a href="/studio" aria-label="Open Sanity Studio"><span class="material-symbols-outlined cursor-pointer hover:animate-pulse">terminal</span></a>
    </div>
  </nav>
